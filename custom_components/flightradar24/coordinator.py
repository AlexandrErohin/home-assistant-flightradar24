...
class FlightRadar24Coordinator(DataUpdateCoordinator[int]):
    ...
    async def _get_flight_data(self, flight: dict) -> dict[str, Any] | None:
        ...
        return {
            ...
            'airport_origin_latitude': self._get_value(flight, ['airport', 'origin', 'position', 'latitude']),
            'airport_origin_longitude': self._get_value(flight, ['airport', 'origin', 'position', 'longitude']),
            ...
            'airport_destination_latitude': self._get_value(flight, ['airport', 'destination', 'position', 'latitude']),
            'airport_destination_longitude': self._get_value(flight, ['airport', 'destination', 'position', 'longitude']),
